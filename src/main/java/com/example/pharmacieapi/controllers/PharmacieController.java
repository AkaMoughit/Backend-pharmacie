...
public class PharmacieController {
	...
	@PostMapping("/add/{user_id}/{zone_id}")
	public Pharmacie save(@PathVariable int user_id, @PathVariable int zone_id, @RequestBody Pharmacie p) {
	    p.setEtat(0);
	    User user = userRepository.findById(user_id).orElseThrow(() -> new IllegalArgumentException("Invalid user ID"));
	    p.setUser(user);
	    p.setZone(zoneService.findById(zone_id).orElseThrow(() -> new IllegalArgumentException("Invalid zone ID")));
